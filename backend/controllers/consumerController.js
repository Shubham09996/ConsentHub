...
const sendRequest = async (req, res) => {
  ...
  try {
    const id = uuidv4();
    await db.query(
      'INSERT INTO consent_requests (id, consumer_id, owner_id, status, purpose) VALUES (?, ?, ?, ?, ?)',
      [id, req.user.id, ownerId, 'PENDING', purpose]
    );
...
const getAccessList = async (req, res) => {
  try {
    const [list] = await db.query(
      `SELECT cr.id, cr.status, u.first_name, u.email, cr.owner_id 
...
const getViewData = async (req, res) => {
  ...
  try {
    ...
    const [consent] = await db.query(
      'SELECT status FROM consent_requests WHERE consumer_id = ? AND owner_id = ?',
      [req.user.id, ownerId]
    );
    ...
    // 2. Fetch Data
    const [record] = await db.query('SELECT data_payload FROM data_records WHERE owner_id = ?', [ownerId]);
    ...
    await db.query(
      'INSERT INTO audit_logs (id, actor_id, action_type, target_id) VALUES (?, ?, ?, ?)',
      [uuidv4(), req.user.id, 'VIEW_DATA', ownerId]
    );
...
const getConsumerDashboardStats = async (req, res) => {
  try {
    ...
    const [activeConnections] = await db.query(
      `SELECT COUNT(*) AS count FROM consent_requests WHERE consumer_id = ? AND status = 'APPROVED'`,
      [req.user.id]
    );

    // Pending Requests
    const [pendingRequests] = await db.query(
      `SELECT COUNT(*) AS count FROM consent_requests WHERE consumer_id = ? AND status = 'PENDING'`,
      [req.user.id]
    );

    // Total API Calls (assuming audit_logs tracks API calls and action_type for VIEW_DATA is an API call)
    const [totalApiCalls] = await db.query(
      `SELECT COUNT(*) AS count FROM audit_logs WHERE actor_id = ? AND action_type = 'VIEW_DATA'`,
      [req.user.id]
    );
...
module.exports = { searchOwner, sendRequest, getAccessList, getViewData, getConsumerDashboardStats, getConsumerApiKey };